...
layui.config({
	...
	function lockPage(){
		layer.open({
			...
			content : '	<div class="admin-header-lock" id="lock-box">'+
							'<div class="admin-header-lock-img"><img src="images/minilogo.png"/></div>'+
	...
	function showNotice(){
		layer.open({
	        ...
	        btn: ['已阅读'],
	        moveType: 1,
	        content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;">图书馆智能座位预约平台系统管理员职责：<br /><br />1、遵循“预约上座，失约无座，拒绝占座”管理理念。<br /><br />2、严格执行图书座位预约管理的规章制度。<br /><br />3、慎重进行各项操作，避免数据出错。</div>',
	        success: function(layero){
				...
				if($(window).width() > 432){  //如果页面宽度不足以显示顶部“系统公告”按钮，则不提示
					btn.on("click",function(){
						layer.tips('系统公告在此处隐藏', '#showNotice', {
							tips: 3
						});
					})
				}
