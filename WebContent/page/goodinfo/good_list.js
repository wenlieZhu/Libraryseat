layui.config({
	...
	$.ajax({
		...
		success: function(data){
			console.log(data.good_json);
		var newArray = [];
		//单击首页“待审核文章”加载的信息
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核文章"){
			if(window.sessionStorage.getItem("goodNews")){
				var goodNews = window.sessionStorage.getItem("goodNews");
				newsData = JSON.parse(goodNews).concat(data.good_json);
			}else{
				newsData = data.good_json;
			}
			...
			if(window.sessionStorage.getItem("goodNews")){
				var goodNews = window.sessionStorage.getItem("goodNews");
				newsData = JSON.parse(goodNews).concat(newsData);
			}
			//执行加载数据的方法
			userinfo_list();
		}
            ...
            setTimeout(function(){
            	$.ajax({
					...
					success : function(data){
						if(window.sessionStorage.getItem("goodNews")){
							var goodNews = window.sessionStorage.getItem("goodNews");
							newsData = JSON.parse(goodNews).concat(data);
						}else{
							newsData = data;
						}
						for(var i=0;i<newsData.length;i++){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				...
				for(var i=0;i<currData.length;i++){
					var gyear=1900+currData[i].gtime.year;
					var gmonth=1+currData[i].gtime.month;
					var gdate=currData[i].gtime.date;
					
					dataHtml += '<tr>'
			    	+'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
			    	+'<td>'+currData[i].goodid+'</td>'
			    	+'<td>'+currData[i].gname+'</td>'
			    	+'<td>'+currData[i].gdetail+'</td>'
			    	+'<td>'+currData[i].gphone+'</td>'
			    	+'<td>'+gyear+'-'+gmonth+'-'+gdate+'</td>';
