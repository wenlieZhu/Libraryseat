layui.config({
	...
	var newsData = '';
	$.ajax({
		url:"https://www.zhuwenlie.cn/Libraryseat/adminServlet?opr=book_list",
		dataType: "json",
		...
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核文章"){
			if(window.sessionStorage.getItem("bookNews")){
				var bookNews = window.sessionStorage.getItem("bookNews");
				newsData = JSON.parse(bookNews).concat(data);
			}else{
				newsData = data;
			}
			...
			if(window.sessionStorage.getItem("bookNews")){
				var bookNews = window.sessionStorage.getItem("bookNews");
				newsData = JSON.parse(bookNews).concat(newsData);
			}
			//执行加载数据的方法
			userinfo_list();
		}
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				...
				for(var i=0;i<currData.length;i++){
					var byear=1900+currData[i].bday.year;
					var bmonth=1+currData[i].bday.month;
					var bdate=currData[i].bday.date;
					
					dataHtml += '<tr>'
			    	+'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
			    	+'<td>'+currData[i].bid+'</td>'
			    	+'<td>'+currData[i].bdate+'</td>'
			    	+'<td>'+currData[i].btime+'</td>'
			    	+'<td>'+byear+'-'+bmonth+'-'+bdate+'</td>'
			    	+'<td>'+currData[i].bremark+'</td>';
			    	if(currData[i].btid == 6001){
			    		dataHtml += '<td>待使用</td>';
			    	}else if(currData[i].btid == 6002){
			    		dataHtml += '<td>已完成</td>';
			    	}else if(currData[i].btid == 6003){
			    		dataHtml += '<td>已取消</td>';
			    	}else if(currData[i].btid == 6004){
			    		dataHtml += '<td>失约</td>';
			    	}else {
			    		dataHtml += '<td>未设置</td>';
			    	}
...
//			    	+'<td>'+currData[i].newsTime+'</td>'
			    	dataHtml +='<td>'
					+  '<a class="layui-btn layui-btn-normal book_edit" data-bid="'+data[i].bid+'"><i class="iconfont icon-edit"></i> 查看详情</a>'
