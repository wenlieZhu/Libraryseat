layui.config({
	...
	$.ajax({
		...
		success: function(data){
			console.log(data.msg_json);
		var newArray = [];
		//单击首页“待审核文章”加载的信息
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核文章"){
			if(window.sessionStorage.getItem("magNews")){
				var magNews = window.sessionStorage.getItem("magNews");
				newsData = JSON.parse(magNews).concat(data.msg_json);
			}else{
				newsData = data.msg_json;
			}
			...
			if(window.sessionStorage.getItem("magNews")){
				var magNews = window.sessionStorage.getItem("magNews");
				newsData = JSON.parse(magNews).concat(newsData);
			}
			//执行加载数据的方法
			userinfo_list();
		}
            ...
            setTimeout(function(){
            	$.ajax({
					...
					success : function(data){
						if(window.sessionStorage.getItem("magNews")){
							var magNews = window.sessionStorage.getItem("magNews");
							newsData = JSON.parse(magNews).concat(data);
						}else{
							newsData = data;
						}
						for(var i=0;i<newsData.length;i++){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				...
				for(var i=0;i<currData.length;i++){
					var myear=1900+currData[i].mtime.year;
					var mmonth=1+currData[i].mtime.month;
					var mdate=currData[i].mtime.date;
					
					dataHtml += '<tr>'
			    	+'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
			    	+'<td>'+currData[i].msgid+'</td>'
			    	+'<td>'+currData[i].mtitle+'</td>'
			    	+'<td>'+currData[i].mdetail+'</td>'
//			    	+'<td>'+currData[i].gphone+'</td>'
			    	+'<td>'+myear+'-'+mmonth+'-'+mdate+'</td>';
